        ...
        with st.form("form_controles"):
            ...
            if r_actual["controles"]:
                ...
                for c in r_actual["controles"]:
                    ...
                    c2.metric("Riesgo Residual Actual", f"{r_actual['total_residual']} / 25", delta=f"
                    {r_actual['total_residual'] - r_actual['total']}", delta_color="inverse")
            else:
                st.warning("Este riesgo aún no cuenta con ningún control aplicado.")
                ==========================================MÓDULO 4: REPORTE E INFORME FINAL==========================================
            elif menu == "4. Reporte e Informe Final":
            st.header("📄 Generación de Informe Técnico de Privacidad")
            st.write("A continuación se presenta el resumen consolidado de tu análisis. Puedes copiar este texto para tu documentación oficial de cumplimiento.")
            st.markdown("---")
            ...
            if not st.session_state.riesgos_evaluados:
                ...
                for idx, r in enumerate(st.session_state.riesgos_evaluados):
                    ...
                    if r["controles"]:
                        ...
                        st.caption("Truco: Haz clic dentro de la caja de texto anterior y presiona Ctrl+A seguido de Ctrl+C para copiarlo.")
