...
if menu == "1. Test de Umbral (EIPD)":
    ...
    with st.expander("➕ Evaluar una nueva amenaza / escenario de riesgo", expanded=True):
        ...
        if st.button("💾 Guardar y Registrar Riesgo"):
            if descripcion_riesgo.strip() == "":
                ...
                nuevo_riesgo = {
                    "amenaza": amenaza_sel, "descripcion": descripcion_riesgo, "probabilidad": probabilidad, "impacto": impacto,
    ...
    if not st.session_state.riesgos_evaluados:
        ...
        with st.form("form_controles"):
            ...
            controles_sugeridos = ["Cifrado de datos en reposo y en tránsito", "Autenticación de Doble Factor (2FA) para accesos", "Políticas estrictas de control de accesos (RBAC)", "Copias de seguridad periódicas y cifradas", "Formación y concienciación periódica al personal", "Acuerdo de Confidencialidad y NDA firmado", "Auditorías técnicas y test de penetración anuales", "Otro control específico"]
            control_tipo = st.selectbox("Medida de seguridad / salvaguarda:", controles_sugeridos)
            control_detalles = st.text_input("Detalles de la implementación:", placeholder="Ej. Se implementará BitLocker en portátiles.")
    ...
    st.write("A continuación se presenta el resumen consolidado de tu análisis.")
    st.markdown("---")
    reporte_texto = f"=== INFORME DE PRIVACIDAD POR DISEÑO Y RIESGOS ===\n\n1. TEST DE UMBRAL (EIPD):\n- Criterios de alto riesgo detectados: {conteo_si_global} / 9\n- Dictamen: {'EIPD OBLIGATORIA' if conteo_si_global >= 2 else 'EIPD RECOMENDABLE' if conteo_si_global == 1 else 'EIPD NO OBLIGATORIA (Riesgo Bajo)'}\n\n2. MATRIZ DE RIESGOS E IMPACTOS:\n"
    if not st.session_state.riesgos_evaluados:
        reporte_texto += " No se han registrado riesgos en esta sesión.\n"
    else:
        for idx, r in enumerate(st.session_state.riesgos_evaluados):
            reporte_texto += f"\n[Riesgo #{idx+1}] Amenaza: {r['amenaza']}\n - Descripción: {r['descripcion']}\n - Riesgo Inicial: {r['total']} ({r['nivel']}) [P:{r['probabilidad']}, I:{r['impacto']}]\n - Controles Aplicados:\n"
            if r["controles"]:
                for c in r["controles"]:
                    reporte_texto += f"   * {c}\n"
            else:
                reporte_texto += f"   * Ninguno registrado\n"
            reporte_texto += f" - Riesgo Residual Final: {r['total_residual']} ({r['nivel_residual']}) [P:{r['prob_residual']}, I:{r['imp_residual']}]\n"
    st.text_area("📋 Copiar Reporte Completo:", value=reporte_texto, height=450)
