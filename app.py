    ...
    if not st.session_state.riesgos_evaluados:
        ...
        with st.form("form_controles"):
            ...
            if st.form_submit_button("🛡️ Aplicar y Actualizar Riesgo"):
                ...
                # Actualizar los datos del riesgo seleccionado en el estado global
...
Usa el código con precaución.st.success("¡Control aplicado y riesgo residual calculado!")st.rerun()# Mostrar estado actual de las mitigaciones de este riesgost.markdown("### 📊 Estado de Mitigación Actual")if r_actual["controles"]:st.write("Controles aplicados:")for c in r_actual["controles"]:st.write(f"- {c}")# Comparativa visual de riesgoc1, c2 = st.columns(2)c1.metric("Riesgo Bruto Inicial", f"{r_actual['total']} / 25", delta=None)c2.metric("Riesgo Residual Actual", f"{r_actual['total_residual']} / 25", delta=f"{r_actual['total_residual'] - r_actual['total']}", delta_color="inverse")else:st.warning("Este riesgo aún no cuenta con ningún control aplicado.")==========================================MÓDULO 4: REPORTE E INFORME FINAL==========================================elif menu == "4. Reporte e Informe Final":st.header("📄 Generación de Informe Técnico de Privacidad")st.write("A continuación se presenta el resumen consolidado de tu análisis. Puedes copiar este texto para tu documentación oficial de cumplimiento.")st.markdown("---")# Construcción de la caja de texto finalreporte_texto = f"=== INFORME DE PRIVACIDAD POR DISEÑO Y RIESGOS ===\n\n"reporte_texto += f"1. TEST DE UMBRAL (EIPD):\n"reporte_texto += f"- Criterios de alto riesgo detectados: {conteo_si_global} / 9\n"reporte_texto += f"- Dictamen: {'EIPD OBLIGATORIA' if conteo_si_global >= 2 else 'EIPD RECOMENDABLE' if conteo_si_global == 1 else 'EIPD NO OBLIGATORIA (Riesgo Bajo)'}\n\n"reporte_texto += f"2. MATRIZ DE RIESGOS E IMPACTOS:\n"if not st.session_state.riesgos_evaluados:reporte_texto += " No se han registrado riesgos en esta sesión.\n"else:for idx, r in enumerate(st.session_state.riesgos_evaluados):reporte_texto += f"\n[Riesgo #{idx+1}] Amenaza: {r['amenaza']}\n"reporte_texto += f" - Descripción: {r['descripcion']}\n"reporte_texto += f" - Riesgo Inicial: {r['total']} ({r['nivel']}) [P:{r['probabilidad']}, I:{r['impacto']}]\n"reporte_texto += f" - Controles Aplicados:\n"if r["controles"]:for c in r["controles"]:reporte_texto += f"   * {c}\n"else:reporte_texto += f"   * Ninguno registrado\n"reporte_texto += f" - Riesgo Residual Final: {r['total_residual']} ({r['nivel_residual']}) [P:{r['prob_residual']}, I:{r['imp_residual']}]\n"st.text_area("📋 Copiar Reporte Completo:", value=reporte_texto, height=450)st.caption("Truco: Haz clic dentro de la caja de texto anterior y presiona Ctrl+A (o Cmd+A) seguido de Ctrl+C para llevártelo a Word, un correo o tu informe en PDF.")
---

### ¿Qué verás ahora en tu web?
En cuanto guardes este código, vuelve a tu pestaña de la aplicación web y actualízala. Verás aparecer todo el ecosistema funcionando en tiempo real:
1. Podrás marcar los "Sí" en el test de umbral.
2. Pasarás al módulo 2 a crear un par de riesgos ficticios (por ejemplo: *"Fuga por phishing"*) y guardarlos.
3. En el módulo 3, podrás seleccionar esos mismos riesgos que acabas de inventarte, asociarles medidas de seguridad y ver cómo baja la gráfica de riesgo residual.
4. En el módulo 4 tendrás el acta de cumplimiento redactada de forma automática.
